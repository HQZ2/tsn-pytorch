...
name_pattern = {'FLOW':'activitynet-frames-flows/{video_name}/flow_{xy}_{idx:05d}.jpg','RGB':'activitynet-frames-flows/{video_name}/image_{idx:05d}.jpg'}
num_segment = 3
num_length = 1
num_epoch = 10
...
def generate_frame_info(video_info, shuffle=True):
	...
	for video_path, video_duration, label in video_info:
		...
		for i in xrange(num_segment):
			frame_rng = random.randint(i*avg_duration,min((i+1)*avg_duration-num_length+1,video_duration-1)-num_length+1)
			for j in xrange(num_length):
				...
				if train_data == 'FLOW':
					frame_path_x = bucket_domain + name_pattern['FLOW'].format(video_name=video_path,xy='x',idx=frame_rng+j)
					frame_path_y = bucket_domain + name_pattern['FLOW'].format(video_name=video_path,xy='y',idx=frame_rng+j)
